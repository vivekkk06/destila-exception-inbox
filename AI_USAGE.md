...
**2.** (Pasted the actual pandas inspection output showing null `planned_units`, duplicate rows, and dirty `sku` casing/whitespace)
...
→ Got a diagnosis that my user wasn't in the `docker` group and the daemon wasn't running, plus the exact `usermod`/`systemctl` commands to fix it — which then surfaced a second, deeper error.
...
**5.** (Pasted the backend container's `PermissionError: [Errno 13] Permission denied: 'data/raw/production_plan.csv'` even after host file permissions were confirmed correct)
